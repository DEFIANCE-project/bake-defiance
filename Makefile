...
bootstrap: $(NS3_HOME)/ns3 | $(NS3_HOME)/contrib/DEFIANCE $(NS3_HOME)/contrib/ai $(NS3_HOME)/contrib/nr
	ns3 configure --enable-test --enable-python --enable-examples --enable-modules=point-to-point,point-to-point-layout,network,applications,mobility,csma,internet,flow-monitor,lte,wifi,energy,ai,DEFIANCE -d debug --out $(NS3_HOME)/build
	ns3 build ai
